...
def generate_figures(df, results_dir, label):
    ...
    for row, scenario in enumerate(scenarios):
        ...
        ax.bar(grouped.index, grouped.values, width=3, color=colors)
    ...
    fig.suptitle(f"P(Congruency) by Visual Size — Shrink ({label})")
    plt.tight_layout()
    ...
    for metric, ylabel, filename in [("time", "Average Time (s)", "time.png"),
        ...
        for row, scenario in enumerate(scenarios):
            ...
            ax.bar(grouped.index, grouped.values, width=3, color='#416cc1')
            ax.set_xticks(visual_sizes)
        ...
        fig.suptitle(f"{ylabel} by Visual Size — Shrink ({label})")
        plt.tight_layout()
